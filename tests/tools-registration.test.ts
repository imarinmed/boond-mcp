...
describe('1. HR Domain - Tool Registrations', () => {
  ...
  describe('Contacts Tools', () => {
    it('should register 5 contact tools', () => {
      ...
      const calls = mockServer.registerTool.mock.calls;
      const contactTools = calls.filter((call: any[]) => call[0].includes('contacts'));
...
describe('9. Total Tool Count Verification', () => {
  ...
  it('should register total of 121 tools across all domains', () => {
    ...
    registerResourceTools(mockServer as any, mockClient);
    registerContractTools(mockServer as any, mockClient);
    registerBulkCreateCandidateTool(mockServer as any, mockClient);
    registerBulkUpdateCandidateTool(mockServer as any, mockClient);
    registerBulkDeleteCandidateTool(mockServer as any, mockClient);
    registerBulkCreateContactTool(mockServer as any, mockClient);
    registerBulkUpdateContactTool(mockServer as any, mockClient);
    registerBulkDeleteContactTool(mockServer as any, mockClient);
    registerBulkCreateResourceTool(mockServer as any, mockClient);
    registerBulkUpdateResourceTool(mockServer as any, mockClient);
    registerBulkDeleteResourceTool(mockServer as any, mockClient);
    registerCompanyTools(mockServer as any, mockClient);
    registerOpportunityTools(mockServer as any, mockClient);
    registerQuotationTools(mockServer as any, mockClient);
    registerBulkCreateCompanyTool(mockServer as any, mockClient);
    registerBulkUpdateCompanyTool(mockServer as any, mockClient);
    registerBulkDeleteCompanyTool(mockServer as any, mockClient);
    registerInvoiceTools(mockServer as any, mockClient);
    registerPurchaseTools(mockServer as any, mockClient);
    registerOrderTools(mockServer as any, mockClient);
    registerBankingTools(mockServer as any, mockClient);
  ...
  it('should verify all tools have descriptions', () => {
    ...
    const allHaveDescriptions = calls.every(
      (call: any[]) =>
        call[1] && typeof call[1].description === 'string' && call[1].description.length > 0
    );
  ...
  it('should verify all tools have input schemas', () => {
    ...
    const calls = mockServer.registerTool.mock.calls;
    const allHaveSchemas = calls.every((call: any[]) => call[1] && call[1].inputSchema);
  ...
  it('should verify all tools have handler functions', () => {
    ...
    const calls = mockServer.registerTool.mock.calls;
    const allHaveHandlers = calls.every((call: any[]) => typeof call[2] === 'function');
...
describe('10. Handler Execution & Error Handling', () => {
  ...
  describe('Domain Tool Consistency', () => {
    it('should have consistent tool naming across HR domain', () => {
      ...
      const calls = mockServer.registerTool.mock.calls;
      const hrDomainCalls = calls.filter(
        (call: any[]) =>
          call[0].includes('_candidates_') ||
          call[0].includes('_contacts_') ||
          call[0].includes('_resources_') ||
          call[0].includes('_contracts_')
      );
