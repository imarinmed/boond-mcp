...
export function registerAgencyTools(server: McpServer, client: BoondAPIClient): void {
  server.registerTool(
  ...
  server.registerTool(
    'boond_agencies_create',
    {
      ...
      inputSchema: createAgencySchema.merge(dryRunSchema).shape,
    },
    async params => {
      try {
        const validated = createAgencySchema.merge(dryRunSchema).parse(params);
        const { dryRun, ...data } = validated;
        if (dryRun) {
          return dryRunResponse('Create Agency', data);
        }
        const agency = await client.createAgency(data);
        const text = formatAgency(agency);
  ...
  server.registerTool(
    'boond_agencies_update',
    {
      ...
      annotations: WRITE_TOOL_ANNOTATIONS,
      inputSchema: updateAgencyWithIdSchema.merge(dryRunSchema).shape,
    },
    async params => {
      try {
        const validated = updateAgencyWithIdSchema.merge(dryRunSchema).parse(params);
        const { id, dryRun, ...updateData } = validated;
        if (dryRun) {
          return dryRunResponse('Update Agency', { id, ...updateData });
        }
